...
| **SaMD classification and IMDRF documentation** | A,B,C | OA intended use mapped to IMDRF significance/situation matrix; SaMD boundary definition in SA system scope | SaMD profile with stereotypes for IMDRF category (I-IV), significance of information, and healthcare situation severity; BDD modeling SaMD boundary | Tagged values on SaMD package capturing IMDRF risk category, software documentation level derivation, and clinical association evidence mapping | SaMD classification module with attributes for IMDRF category, significance level, healthcare situation, and trace links to clinical evidence records and software documentation level justification | SaMD functional model; algorithm performance requirements linked to IMDRF significance categories |
| **Usability engineering (IEC 62366-1)** | B,C | OA user profiles and use scenarios; SA use-related risk analysis inputs linked to ISO 14971 hazard identification; PA user interface elements traced to formative/summative evaluation records | Use specification stereotypes on actor blocks; use-related risk analysis as Activity Diagrams feeding ISO 14971 hazardous situations; formative evaluation records linked to design iteration baselines; summative test case elements with Verify relationships to critical task requirements | Use environment elements with tagged values for user population, use scenario state machines, use error analysis linked to risk table rows, and summative evaluation test suite linked to critical task list | Use specification module; critical task attributes with severity linkage; formative evaluation result attributes per design iteration; summative evaluation protocol/report modules with bidirectional trace to use-related risk controls in risk module | User interface model blocks; critical task simulation; HMI prototype linkage to evaluation records |
...
| **SaMD clinical association evidence** | OA clinical workflow scenarios linking SaMD output to clinical decision; SA performance requirements with sensitivity/specificity/AUC targets traced to IMDRF significance category | Clinical workflow Activity Diagrams; performance requirement blocks with tagged values for reference standard, target population, and performance metrics; traceability from IMDRF category to depth of clinical evidence required | Clinical association scenario elements; algorithm performance requirements with tagged values for validation dataset characteristics, subgroup definitions, and statistical acceptance criteria | Clinical evidence module; algorithm validation attributes (reference standard, sample size, confidence intervals); trace from IMDRF risk category to clinical evidence depth; linked validation dataset characterization records | Forward from IMDRF classification to clinical evidence requirements; backward from algorithm validation results to residual risk determination |
| **Usability-related risk controls (IEC 62366-1 / ISO 14971 integration)** | Use-related hazardous situations identified in OA user scenarios traced to ISO 14971 risk file; risk controls allocated in LA/PA user interface design with trace to summative evaluation | Use error analysis stereotypes on interaction elements; use-related hazardous situation elements linked bidirectionally to ISO 14971 hazard elements; summative test case Verify relationships to use-related risk controls | Use error elements with tagged values for task criticality, error type, and consequence severity; links from use-related risk controls to both ISO 14971 risk table rows and IEC 62366-1 summative evaluation test cases | Use-related risk module bridging IEC 62366-1 critical task list to ISO 14971 hazardous situations; formative evaluation result attributes feeding design iteration; summative evaluation trace to use-related residual risk determination | Bidirectional: use-related hazard ↔ risk control ↔ UI design ↔ summative evaluation ↔ residual risk |
...
| **SaMD documentation level justification** | OA intended use and clinical context mapped to IMDRF significance/situation matrix; SA software function risk profile | Left arm: risk-based determination | OA clinical workflow scenarios; SA software function hazard contribution analysis; IMDRF classification attributes on SaMD package | Software documentation level determination rationale; IMDRF category mapping; crosswalk from software function risk profile to Basic vs Enhanced documentation scope per FDA guidance "Content of Premarket Submissions for Device Software Functions" |
| **Usability validation (IEC 62366-1 summative)** | Post-PA (user environment) | Right arm: validation | OA critical task list derived from use-related risk analysis; summative evaluation plan referencing user profiles, use environments, and critical tasks from OA | Summative usability evaluation protocol and report; critical task completion data; use error analysis results; trace from summative findings to use-related residual risk acceptability in ISO 14971 risk file |
...
| **SaMD IMDRF classification evidence** | OA intended use significance mapping; SA clinical workflow analysis determining healthcare situation category | IMDRF classification matrix export; SaMD boundary diagram; clinical association evidence summary linking algorithm output to clinical decision context | SaMD classification rationale document; IMDRF category justification; software documentation level derivation from IMDRF risk category |
| **Usability engineering file (IEC 62366-1)** | OA user profiles, use environments, and use scenarios; SA use-related risk analysis outputs; LA/PA user interface design elements with formative evaluation traces | Use specification export; critical task list; formative evaluation records per design iteration; summative evaluation protocol/report with critical task pass/fail data | Usability engineering file; use specification document; formative evaluation summary; summative usability evaluation report with trace to use-related residual risk acceptability |
...
**7. Software change documentation gaps breaking the change control loop**
- Mistake: Fixing software defects through the development process (bug fix, code review, merge) without completing the full change control sequence required by 21 CFR 820.30(i) and IEC 62304 Clause 6.2. The code gets fixed and tested, but the change request is never linked to the originating software problem report, the risk impact analysis is not updated to reflect whether the defect introduced or modified a hazardous situation, and the design history file is not updated with the re-verification evidence.
- Why wrong: FDA premarket inspectors and post-market 483 observations trace the chain from software problem report (IEC 62304 Clause 9) through change request (IEC 62304 Clause 6.2.1), risk impact analysis (IEC 62304 Clause 6.2.2, ISO 14971 Clause 7), re-verification of affected items (IEC 62304 Clause 6.2.4), and design history file update (21 CFR 820.30(j)). If any link is missing, the change is undocumented from a regulatory perspective even if the code is correct. Teams that route bug fixes through development tooling (Jira, GitHub) without feeding back into the design control and risk management systems produce a DHF that does not reflect the actual design history. FDA inspectors specifically look for software changes that bypassed change control -- it is one of the most common 483 observations for software-intensive devices.
- Correct approach: Every software problem report that results in a code change must generate a formal change request traceable to the problem report. The change request must include a risk impact analysis per IEC 62304 Clause 6.2.2 determining whether the change affects the software safety classification, introduces new hazards, or modifies existing risk controls. Re-verification must cover all affected software items per the regression analysis, and the results must be recorded in the DHF. The risk management file must be updated if the change affects any hazardous situation, risk control, or residual risk determination. The software configuration management system must link the problem report, change request, affected baselines, and verification records into a single traceable chain.

**8. SOUP/OTS risk management gaps under QMSR alignment to ISO 13485**
- Mistake: Managing SOUP/OTS components using legacy QSR (pre-QMSR) practices that treated off-the-shelf software as a purchasing control issue (21 CFR 820.50) without the lifecycle rigor required by IEC 62304 Clause 8 and the QMSR's alignment to ISO 13485:2016 Clause 7.4. Teams list SOUP components, record the version at integration time, and move on -- no ongoing anomaly list monitoring, no version-specific risk assessment when updating SOUP, and no integration testing evidence demonstrating the SOUP component functions correctly within the device software architecture.
- Why wrong: The QMSR alignment to ISO 13485:2016 raises the bar on externally provided processes, products, and services (Clause 7.4), and IEC 62304 Clause 8 requires specific lifecycle activities for each SOUP item: functional and performance requirements (8.1.2), evaluation of published anomaly lists for risk impact (8.1.2), and ongoing monitoring of anomaly sources (8.1.2 maintenance). FDA reviewers now expect to see a SOUP management plan that names the anomaly sources monitored (NVD, vendor security advisories, CVE databases), documents the evaluation cadence, and demonstrates that each SOUP version used in the released device has a version-specific anomaly evaluation on file. When a SOUP component is updated, the change must trigger the IEC 62304 Clause 6.2 change process including risk impact analysis and regression testing of integration points. Teams using the legacy QSR framework often treat SOUP updates as routine maintenance rather than controlled design changes, producing no evidence that the new SOUP version was evaluated for safety impact before deployment. FDA has cited this pattern in 483 observations where the SOUP anomaly evaluation was either absent, performed only at initial integration without updates, or covered the SOUP component generically without version-specific analysis.
- Correct approach: Maintain a SOUP register that records for each item: component name, version, manufacturer/source, functional requirements the device depends on, performance requirements, anomaly sources monitored, date of last anomaly evaluation, and risk impact determination. When a SOUP component is updated, execute the full IEC 62304 Clause 6.2 change process: document the change request, perform risk impact analysis evaluating whether the new version introduces new anomalies or resolves previously accepted ones, execute integration testing at all SOUP integration points identified in the software architecture (IEC 62304 Clause 5.1.7), and update the risk management file if the SOUP change affects any hazardous situation. The SOUP monitoring process must be documented in the software maintenance plan with defined frequency and responsibility, and monitoring evidence must be retained as quality records per ISO 13485:2016 Clause 4.2.5.
...
### Engagement Example: Class II SaMD 510(k) Premarket Submission Preparation

A team is preparing a 510(k) submission for a Class II SaMD product -- a clinical decision support tool that analyzes diagnostic imaging data and provides risk stratification outputs to clinicians. The SaMD output drives clinical management decisions for a serious healthcare situation. Under the IMDRF framework, this places the product at Category III (drive clinical management + serious situation), which maps to Class II under FDA's classification and triggers Enhanced software documentation level per the current FDA guidance "Content of Premarket Submissions for Device Software Functions."

**Step 1: Design History File Content Assessment**

The DHF for this submission must contain, at minimum:
- Design and development plan per 21 CFR 820.30(b) / ISO 13485:2016 Clause 7.3.2, including all revisions reflecting scope changes during development
- Design input records per 21 CFR 820.30(c): user needs, intended use statement, performance requirements (algorithm accuracy targets with statistical acceptance criteria), safety requirements derived from ISO 14971 risk analysis, cybersecurity requirements from the threat model, and IEC 62366-1 use specification
- Design output records per 21 CFR 820.30(d): software requirements specification, software architecture design, detailed design (required because IEC 62304 safety classification for this product is Class C -- the algorithm's failure to correctly stratify risk can contribute to a hazardous situation resulting in serious injury through delayed or incorrect treatment), interface control documents, and SOUP/OTS component specifications
- Design review records per 21 CFR 820.30(e) at each lifecycle phase gate, with documented attendees, action items, and closure evidence
- Design verification records per 21 CFR 820.30(f): unit test results, integration test results (including SOUP integration points), system test results, and traceability matrix demonstrating every design input has a corresponding verification record
- Design validation records per 21 CFR 820.30(g): clinical validation data demonstrating algorithm performance on a dataset representative of the intended patient population, usability validation per IEC 62366-1 summative evaluation, and evidence that validation was performed under conditions representative of actual use
- Design transfer records per 21 CFR 820.30(h): evidence that the production build process, deployment pipeline, and configuration management reproduce the validated design output
- Design change records per 21 CFR 820.30(i): every change after initial design freeze, with risk impact analysis and re-verification evidence
- Risk management file per ISO 14971: complete from risk management plan through residual risk evaluation and risk management report
- Software lifecycle records per IEC 62304: software development plan, SOUP management records, anomaly management records, and configuration management records

**Step 2: Software Documentation Package -- Enhanced Level**

Because the documentation level is Enhanced, the 510(k) submission must include the full software documentation set beyond what Basic level requires:
- **Software description**: Intended use, indications for use, device software functions, system overview, and operating environment
- **Software risk analysis**: Summary of software-related hazards, hazardous situations, risk controls, and residual risks. This must demonstrate integration with the ISO 14971 risk file, not be a standalone analysis
- **Software requirements specification (SRS)**: Complete set of software requirements including functional, performance, interface, safety, and cybersecurity requirements. Each requirement must be uniquely identifiable and traceable to its source (user need, risk control, regulatory requirement)
- **Architecture design chart**: Software architecture showing subsystem decomposition, SOUP integration points, data flow, control flow, and the isolation boundaries that support the safety classification. For this SaMD, the architecture must clearly show the algorithm processing pipeline, data input interfaces, output presentation to the clinician, and any cloud or network components that cross the regulatory boundary
- **Software design specification**: Detailed design for each software unit (Class C per IEC 62304 Table A.1). For the algorithm core, this includes the mathematical specification, feature extraction pipeline, model architecture (if ML-based), and decision thresholds
- **Traceability analysis**: Bidirectional matrix linking requirements → architecture → detailed design → implementation → unit tests → integration tests → system tests. FDA reviewers will walk specific rows to confirm the chain is unbroken. Gaps in traceability are the single most common software review deficiency
- **Unresolved anomalies list**: Every known software anomaly that remains open at the time of submission, with risk impact assessment per IEC 62304 Clause 9 and a justification for why each unresolved anomaly is acceptable for release
- **Revision-level history**: Complete version history of the software, including all changes from initial development through the submitted release, with references to the change control records in the DHF

**Step 3: Reviewer Challenge Points**

For this SaMD 510(k), FDA reviewers will predictably challenge:

1. **IMDRF classification justification**: The reviewer will verify that the IMDRF significance category (drives clinical management) and healthcare situation (serious) are correctly assigned. If the team claims "informs" rather than "drives," the reviewer will examine the intended use statement and labeling to determine whether the SaMD output is the primary basis for the clinical decision. Getting this wrong changes the documentation level and the entire evidence package.

2. **Algorithm validation dataset representativeness**: The reviewer will ask whether the validation dataset is representative of the intended patient population (demographics, disease prevalence, imaging equipment variability). A dataset drawn from a single site or a non-representative population will trigger a deficiency letter. The statistical analysis plan must prespecify performance targets, sample size justification, and subgroup analyses.

3. **Software documentation level derivation**: The reviewer will trace from the software function description through the risk profile to the Basic/Enhanced determination. If the team selected Basic when the software directly drives a clinical management decision for a serious condition, the reviewer will reject the documentation level and require Enhanced documentation, which means the submission is incomplete.

4. **SOUP anomaly evaluation completeness**: For Enhanced documentation, the reviewer will examine the SOUP register and verify that each SOUP component has a version-specific anomaly evaluation. The reviewer will cross-check the SOUP versions in the SBOM against the anomaly evaluations on file. Missing evaluations for SOUP components in the algorithm pipeline or data processing stack will generate a deficiency.

5. **Cybersecurity documentation completeness**: Per FDA's "Cybersecurity in Medical Devices" guidance, the reviewer expects a threat model reflecting the SaMD's connectivity profile, an SBOM in a machine-readable format, a vulnerability management plan, and evidence that cybersecurity controls are integrated into the ISO 14971 risk file. SaMD products that process patient data or connect to hospital networks face heightened scrutiny on data integrity and confidentiality controls.

6. **Change control loop closure**: The reviewer will sample design changes from the revision-level history and trace each through the change request, risk impact analysis, re-verification, and DHF update. Changes that lack any link in this chain -- particularly bug fixes that went through development tooling without completing the formal change control process -- will generate a deficiency or, in a premarket inspection, a 483 observation per 21 CFR 820.30(i).
